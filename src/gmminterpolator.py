...
class GMMInterpolator:
    def __init__(
        self, t_start: float, dt: float, ps: List[torch.Tensor], ms: List[torch.Tensor]
    ) -> None:
    ...
    def _iterpolate_between(
        ...
        i = 0
        A_cons.resize(
            A_cons.shape[0] - 1, A_cons.shape[1]
    ...
    def save_coeff(self, path: str):
        ...
        if not os.path.exists(os.path.dirname(str)):
            os.makedirs(os.path.dirname(str))
    ...
    @classmethod
    def walking_man(
        cls, t_start: float, L: int, resolution: Tuple[int, int] = (64, 64)
    ) -> "GMMInterpolator":
