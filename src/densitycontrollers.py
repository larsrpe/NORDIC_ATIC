...
class GausianController(DensityController):
    ...
    def get_contoll(self, t:float) -> torch.Tensor:
        ...
        #feedforward
        if (
            self.use_ff  and #use feedforward control
            m.requires_grad #m is a function of time
            ):
            comps = self.gmm.get_components_batch(self.state)
            feed_foward= torch.zeros_like(self.state)
            for i in range(self.state.size(0)):
                for j in range(2):
                    feed_foward[i,j]=torch.autograd.grad(m[:,j],t_tensor,comps[i],retain_graph=True)[0]
        else:
            feed_foward = 0
...
class WalkingManController(DensityController):
    ...
    def __init__(self,L: float,h:float,D:float,sigma_pixels=1,t_start=1,resolution=(64,64),use_ff: bool = True) -> None:
        ...
        try:
            self.gmminterpolator.load_coeff(f'data/walking_man/resolution{resolution[0]}')
        except: 
            print("computing interpolation coeffs, this might take a while...")
            self.gmminterpolator.interpolate()
            self.gmminterpolator.save_coeff(f"data/walking_man/resolution{resolution[0]}")
        #repeat three times
    ...
    def get_contoll(self, t:float) -> torch.Tensor:
        ...
        if (
            ...
            for i in range(self.state.size(0)):
                for j in range(2):
                    feed_foward[i,j]=torch.autograd.grad(m[:,j],t_tensor,comps[i],retain_graph=True)[0] # try with minus here
        else:
            feed_foward = 0 
