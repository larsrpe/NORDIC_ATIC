...
def viz_tracking(t,Y,L,pdf: Callable[[float,torch.Tensor],torch.Tensor],num_points: int,file_name = "test"):
    fig = plt.figure() 
    ...
    def animate(i):
        ...
        z = z.numpy()[::-1,:]
        plt.clf()
        return plt.imshow(z, cmap='hot', interpolation='nearest',alpha=0.5,extent=[0,L,0,L]),plt.plot(x,y,"ko",markersize=5)
