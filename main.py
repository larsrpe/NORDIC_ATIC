...
def demo_gaussian():
    ...
    D = 5  # feedback gain
    X0 = torch.rand(N, 2) * torch.tensor([1.5 * L / 4, L / 4]) + torch.tensor(
        [L / 2 * (1 - 1.5 / 4), L / 8]
    )  # initial position
    t_eval = np.linspace(
        0, T, int(60 * T) + 1
    )  # timesteps where we get the simulation results
    device = (
        "cuda"
    ...
    for use_ff in [False, True]:
        controller = GausianController(
            L, h, D, t_start, t_start + 0.5, use_ff=use_ff
        ).to(device)
...
def demo_walking_man():
    ...
    h = L / 20  # smoothing param of density estimation
    D = 5  # feedback gain
    X0 = torch.tensor([4, L]) * torch.rand(N, 2) + torch.tensor(
        [3, 0]
    )  # initial position
    t_eval = np.linspace(
        0, T, int(60 * T) + 1
    ...
    # run sims
    for use_ff in [False, True]:
        controller = WalkingManController(
            L, h, D, t_start, t_start + 0.5, resolution=(64, 64), use_ff=use_ff
        ).to(device)
        t, y = sim(controller, X0, t_eval)
        str = "with" if use_ff else "without"
